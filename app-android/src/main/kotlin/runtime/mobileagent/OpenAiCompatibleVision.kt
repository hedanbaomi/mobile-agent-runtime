...
class OpenAiCompatibleVision(
    ...
    private companion object {
        /**
         * Provider terminal codes that describe a decided outcome.  They are
         * surfaced as `VisionOutcome.Failed` so the import state records the
         * real cause (and a bounded retry range) instead of the blanket
         * `UnknownOutcome` that forbids automatic retry and hides the reason.
         */
        val DECIDED_VISION_FAILURES = setOf(
            ErrorCode.PROVIDER_UNAUTHORIZED.name,
            ErrorCode.INVALID_CONFIG.name,
            ErrorCode.INPUT_OVERFLOW.name,
            ErrorCode.OUTPUT_TRUNCATED.name,
            ErrorCode.REASONING_EXHAUSTED.name,
            ErrorCode.CONTEXT_OVERFLOW.name,
