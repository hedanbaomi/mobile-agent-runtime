...
> **2026-09-25 对话交互补充**：同一用户消息引发的助手分轮文字与工具摘要按发生顺序置于同一助手气泡，思考内容合并在气泡顶部的折叠区；工具确认保留对话可见，使用半透明灰色遮罩和底部确认卡，遮罩拦截底层点击，卡片避开 IME 与导航栏。发送时先收起输入法；Agent 编辑页新增默认关闭的“跳过工具运行确认”开关，说明其仅对新会话生效，关闭则立即收紧。具体设备视觉与后台行为仍须按验收矩阵核验。

> **2026-09-25 工作区权限状态**：Agent 编辑页将旧策略版本的授权显示为“策略已变更，需重新授权”，完整设备文件开关在对应能力授权失效时恢复未勾选状态；重新勾选须经过显式确认。危险模式关闭时，即使历史完整设备授权仍保留，也明确显示该类工具已暂停。普通默认目录与完整设备文件的授权状态分别呈现，避免把系统连接可用误认为 Agent 工具已获授权。
...
- **详情包含**:
  ...
  - 独立的“卸载技能”操作与确认弹窗，逐项说明安装、授权、当前 Agent 绑定会移除，历史对话、审计与专属持久记忆会保留；原包字节仅在历史导出仍需要时保留。
...
- **展示方式**: 按照发布时间逆序排列的卡片流。
- **顶部**: 状态过滤器（全部 / 未读 / 历史）与一键标为已读按钮。四个控件等宽同高；中文“全部标为已读”在 320dp 及以上常规手机宽度内按可用宽度调整字号，单行完整显示。
- **公告卡片**:
