...
2026-09-26 普通工作区只读规则：选择只读预设时撤销该 Agent、该工作区当前普通整目录的写入/删除/补丁能力；新的附加与默认工作区选择只授目录枚举、列表、状态和读取。路径限定、一次性、任务/会话、Skill 和其他工作区的授权不受此撤销操作影响；完整设备文件仍需独立显式确认，不能由只读预设或默认目录附加隐式产生。历史会话的导入凭据只在同 Provider 目标匹配时运行时叠加，不改冻结的模型、工具授权或快照持久内容。
...
用户确认卸载某个安装 ID 后，同一事务从当前 Agent 配置解绑该 ID、撤销旧式与类型化 Grant、使待用批准过期，并移除安装记录。不可变会话快照、调用审计和该安装 ID 的持久 Skill memory 保留；重新导入相同包会生成新的安装 ID，旧授权和记忆不会自动继承。包元数据保留用于历史识别；若无其他安装和历史会话引用，清理原包字节，历史会话仍引用时保留字节供用户显式导出。卸载不删除其他 Skill 或共享知识库。

该数据保留与授权边界的取舍见 [ADR 0012](adr/0012-skill-uninstall-history-boundary.md)。
