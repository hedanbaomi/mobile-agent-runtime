...
> **2026-09-25 危险模式与工作区权限回归**：改变危险模式后，旧 `policyVersion` grant 按既有安全规则失效；Agent 编辑器显式标出并允许在当前版本重新授权。新会话默认工作区解析只接受当前版本的持久授权，避免“已绑定但零工具”。完整设备文件继续由独立的持久高风险 Grant 控制，不作为 Thread 默认；新 Run 可同时纳入该 Agent 已确认的完整设备范围和 Thread 所选的普通目录，均按 selected Authority、危险模式、策略版本及实时撤权复核。再次明确确认完整设备文件时只按默认目录已有且未撤销、未过期的能力集重新确认，不能补发曾撤销的写/删权限，也不会补发 shell；若目录续期失败，完整设备授权的成功状态须单独呈现。运行诊断写入聚合的 grant、backend 与 Authority 计数，便于区分授权失效和后端不可用。
...
| Skills | 来源/许可/签名/兼容性/源码/权限；安装、禁用、卸载、撤销和调用日志；历史与持久数据保留范围可见 |
| Announcements | 固定入口、未读、历史、横幅、重要确认；标全部已读单行等宽；失败不影响其他能力 |
